...
void ST7735_DrawMyCharS(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
  uint8_t line; // vertical column of pixels of character in font
  int32_t i, j;
  if((x >= _height )            || // Clip right
     (y >= _width)           || // Clip bottom
     ((x + 8 * size - 1) < 0) || // Clip left
     ((y + 5 * size - 1) < 0))   // Clip top
    return;
  ...
  for (i=0; i<6; i++ ) {
    ...
    for (j = 0; j<5; j++) {
      if (line & 0x1) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+j, y+i, textColor);
        else {  // big size
          ST7735_FillRect(x+(j*size), y+(i*size), size, size, textColor);
        }
      } else if (bgColor != textColor) {
        if (size == 1) // default size
          ST7735_DrawPixel(x+j, y+i, bgColor);
        else {  // big size
          ST7735_FillRect(x+j*size, y+i*size, size, size, bgColor);
        }
      }
      line >>= 1;
    }
...
uint32_t ST7735_DrawmyString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawMyCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
    pt++;
    x = x+1;
