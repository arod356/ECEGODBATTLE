...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		...
		LDR R1, =DC				
		LDR R2, [R1]
		BIC R2, R2, #0x40		;3) Clear DC=PA6 to zero
		STR R2, [R1]
		;LDR R0, =DC_COMMAND
		;LDR R0, [R0]
		LDR R3, =SSI0_DR_R
		STR R0, [R3]			;4) Write the command to SSI0_DR_R
disp	LDR R1, =SSI0_SR_R		;5) Read SSI0_SR_R and check bit 4,
		LDR R2, [R1]
		AND R3, R2, #0x10
		CMP R3, #0x10			;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		BEQ disp
